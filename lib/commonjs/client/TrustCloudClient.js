...
exports.TrustCloudClient = void 0;
var _auth = require("./auth");
var _environments = require("./environments");
var _errors = require("./errors");
class TrustCloudClient {
  constructor(config) {
  ...
  async createSignSession(params) {
    ...
    if (!signerRef) {
      throw new _errors.TrustCloudApiError('createSignSession needs at least one signer (or an explicit identityClientReference)', 0, '');
    }
    const {
      trustCloudFileId
  ...
  resolveUseCaseId(flow, override) {
    ...
    if (!id) {
      throw new _errors.TrustCloudApiError(`No useCaseId configured for '${flow}' — pass useCaseId or set useCaseIds.${flow} in the client config`, 0, '');
    }
    return id;
  }
  async request(method, path, body) {
    ...
    if (!res.ok) {
      throw new _errors.TrustCloudApiError(`TrustCloud request failed: ${method} ${path} → ${res.status}`, res.status, path, await (0, _errors.safeJson)(res));
    }
...
exports.TrustCloudClient = TrustCloudClient;
