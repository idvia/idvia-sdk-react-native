...
import { TrustCloudApiError, safeJson } from './errors';
export class TrustCloudClient {
  constructor(config) {
  ...
  async createSignSession(params) {
    ...
    if (!signerRef) {
      throw new TrustCloudApiError('createSignSession needs at least one signer (or an explicit identityClientReference)', 0, '');
    }
    const {
      trustCloudFileId
  ...
  resolveUseCaseId(flow, override) {
    ...
    if (!id) {
      throw new TrustCloudApiError(`No useCaseId configured for '${flow}' — pass useCaseId or set useCaseIds.${flow} in the client config`, 0, '');
    }
    return id;
  }
  async request(method, path, body) {
    ...
    if (!res.ok) {
      throw new TrustCloudApiError(`TrustCloud request failed: ${method} ${path} → ${res.status}`, res.status, path, await safeJson(res));
    }
...
//# sourceMappingURL=TrustCloudClient.js.map