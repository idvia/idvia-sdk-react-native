...
import { ENVIRONMENT_URLS, TOKEN_PATHS } from './environments';
import { IdviaApiError, safeJson } from './errors';
export class IdviaClient {
  constructor(config) {
    const presets = ENVIRONMENT_URLS[config.environment];
    this.urls = {
      ...presets,
      ...config.urls
    };
    this.useCaseIds = config.useCaseIds ?? {};
    this.auth = new TokenManager(this.urls.identityServer + TOKEN_PATHS[config.environment], config.clientId, config.clientSecret);
  }
  async createVideoIdUnassisted(params) {
    ...
    return {
      ...
      // the API's VideoIdResponseModel serializes as 'videoidentificationId' (lowercase i) — normalize it
  ...
  async createSign(params) {
    ...
    // The API returns the new trustCloudFileId as a bare JSON string ("8a46…"),
    // not the documented { trustCloudFileId } object; accept both.
    return {
      trustCloudFileId: typeof raw === 'string' ? raw : raw.trustCloudFileId
    };
  }
  ...
  async createSignSession(params) {
    ...
    if (!signerRef) {
      throw new IdviaApiError('createSignSession needs at least one signer (or an explicit identityClientReference)', 0, '');
    }
    const {
      trustCloudFileId
    } = await this.createSign(createParams);
    // The provider builds the embedded ceremony asynchronously: until it exists the url
    // endpoint answers 400 "No url found for operation" (about 5 s in practice). Poll it.
    for (let attempt = 1;; attempt++) {
      try {
        ...
        if (!isUrlNotReady(e) || attempt >= urlRetries) throw e;
  ...
  resolveUseCaseId(flow, override) {
    ...
    if (!id) {
      throw new IdviaApiError(`No useCaseId configured for '${flow}' — pass useCaseId or set useCaseIds.${flow} in the client config`, 0, '');
    }
    return id;
  }
  async request(method, path, body) {
    ...
    if (!res.ok) {
      throw new IdviaApiError(`Idvia request failed: ${method} ${path} → ${res.status}`, res.status, path, await safeJson(res));
    }
...
function isUrlNotReady(e) {
  return e instanceof IdviaApiError && e.status === 400 && /no url found/i.test(typeof e.body === 'string' ? e.body : JSON.stringify(e.body ?? ''));
}
function sleep(ms) {
