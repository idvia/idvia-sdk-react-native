...
The real outcome is computed by TrustCloud's backend and delivered to **your**
...
TrustCloud POSTs events to a callback URL configured for your use case. Your
...
| Event | Meaning |
|-------|---------|
| `TrustCloudVideoIDUnassisted` | Flow finished/abandoned. `Message` = `OK`, `KO: USER ABANDONED`, `KO: VIDEOIDENTIFICATION DATE EXPIRED`, … |
...
Example `TrustCloudVideoIDUnassisted` payload:

```json
{
  "VideoUnassistedIdentificationId": "caeb2d39-...",
  "Status": "TrustCloudVideoIDUnassisted",
  "TrustCloudFileId": "7177dc4e-...",
  "UseCaseId": "5f9143c5-...",
...
Poll from your **backend**, not the app (the app has no TrustCloud token). (For
development and pilot integrations using [TrustCloudClient](client.md), the
...
When you use the in-app-browser mode (`openTrustCloudSession`), completion is
